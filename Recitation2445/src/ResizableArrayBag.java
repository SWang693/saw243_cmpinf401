...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
	...
	public BagInterface<T> union(BagInterface<T> anotherBag) {
		...
		int oldBagSize = bag.length;
		int newBagSize = anotherBag.getCurrentSize();
		ResizableArrayBag<T> argBag = (ResizableArrayBag<T>) anotherBag;
		ResizableArrayBag<T> combinedBag = new ResizableArrayBag<T>(oldBagSize + newBagSize);
		for(int i = 0; i < combinedBag.getCurrentSize(); i++) {
			combinedBag.add(bag[i]);
			
		}
		for(int i = 0; i < combinedBag.getCurrentSize(); i++) {
			combinedBag.add(argBag.bag[i]);
		}
	...
	public BagInterface<T> intersection(BagInterface<T> anotherBag) {
		...
		for(int i = 0; i < argBag.getCurrentSize(); i++) {
			if(argBag.contains(bag[i])) {
				arrayCounter++;
			}
		}
		ResizableArrayBag<T> intersectionBag = new ResizableArrayBag<T>(arrayCounter);
		for(int i = 0; i < argBag.getCurrentSize(); i++) {
			if(argBag.contains(bag[i])) {
	...
	public BagInterface<T> difference(BagInterface<T> anotherBag) {
		...
		ResizableArrayBag<T> differenceBag = new ResizableArrayBag<T>(arrayCounter);
